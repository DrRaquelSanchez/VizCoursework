...
Prepare a plot of one categorical variable of your choosing. 
If you do not have one yet, practise the next exercise:
  
The variable "Free Lunch" from the table used in this session is telling you how many students receive lunch in a school. 
If you request a summary you get:
  ```{r, eval=TRUE}
...
plot7 = base2 + geom_bar(fill ="gray",
                        stat = 'identity') 
plot7
...
plot12 = plot11 + geom_text(vjust=0, #hjust if flipping
                          ...
                          aes(y = Percent ,
                              label = LABELS))
plot12 # + coord_flip() # wanna flip the plot?

tableFreq2=tableFreq2[order(tableFreq2$Percent),]
# then:
tableFreq2
(FreeLunchGroup=tableFreq2[order(tableFreq2$Percent),'FreeLunch'])
LABELS=paste0(round(tableFreq2$Percent,2), '%')
base2= ggplot(data = tableFreq2, aes(x = FreeLunch, y = Percent))
